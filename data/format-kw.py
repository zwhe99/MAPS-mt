...
def main(args):
    ...
    with open(os.path.join(raw_dir, f"{test_name}.{src}-{tgt}.{src}")) as test_src_f, \
        open(os.path.join(model_out_dir, f"{test_name}.{src}-{tgt}.{src}.5-shot.ask-kw.trans")) as kw_f:

        test_src_lines = [l.strip() for l in test_src_f.readlines()]
        kw_lines = [l.strip() for l in kw_f.readlines()]

        out_file_path = os.path.join(output_dir, f"{test_name}.{src}-{tgt}.{src}.kw.{seed}-seed")
        demos = []

        with open(out_file_path, 'w') as out_f:
            for id, (src_line, kw_line) in enumerate(zip(test_src_lines, kw_lines)):
                all_items = demos + [(src_line, None, kw_line)]
                prompt_lst = []
                for it in all_items:
                    it_src, it_tgt, it_kw = it
                    s = f"Keyword Pairs: {it_kw}\n\n" + \
