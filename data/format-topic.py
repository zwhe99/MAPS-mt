...
def main(args):
    ...
    with open(os.path.join(raw_dir, f"{test_name}.{src}-{tgt}.{src}")) as test_src_f, \
        ...
        out_file_path = os.path.join(output_dir, f"{test_name}.{src}-{tgt}.{src}.topic.{seed}-seed")
        demos = []

        with open(out_file_path, 'w') as out_f:
            for id, (src_line, topic_line) in enumerate(zip(test_src_lines, topic_lines)):
                all_items = demos + [(src_line, None, topic_line)]
                prompt_lst = []
                for it in all_items:
                    it_src, it_tgt, it_topic = it
                    s = f"Topics: {it_topic}\n\n" + \
